...
namespace Sources.GamePlayer.Player
{
    public class FirstPersonController : MonoBehaviour
    {
        ...
        public bool canCameraLurch = false;
        public float cameraLurchStrength = 2f;
        public float cameraLurchDamping = 0.4f;
        ...
        public float sprintCooldownDelay = .5f;
        ...
        private float currentSprintCooldownDelay;
        ...
        private void Construct(IPlayerInput playerInput)
        {
            ...
            void SubscribeToJumpInput()
            {
                _playerInput.Jumping
                    .Where(x => x && enableJump && isGrounded)
                    .Subscribe(x => Jump())
                    .AddTo(_disposables);
            }
        }
        ...
        private void Update()
        {
            ...
            if (cameraCanMove)
            {
                deltaYaw = _playerInput.CameraRotation.Value.x * Time.deltaTime * mouseSensitivity;

                pitch -= mouseSensitivity * _playerInput.CameraRotation.Value.y * Time.deltaTime;

                // Clamp pitch between lookAngle
                pitch = Mathf.Clamp(pitch, -maxLookAngle, maxLookAngle);

                transform.localEulerAngles = new Vector3(0, transform.localEulerAngles.y + deltaYaw, 0);

                var angles = playerCamera.transform.localEulerAngles;
                playerCamera.transform.localEulerAngles = new Vector3(pitch, angles.y, angles.z);
            }
            ...
            if (enableZoom)
            {
                ...
                if (!isSprinting)
                    isZoomed = _playerInput.Zoom.Value;
            ...
            if (cameraCanMove)
            {
                float targetLurch = deltaYaw * cameraLurchStrength / Time.deltaTime;
                float damp = 1f - Mathf.Pow(cameraLurchDamping, Time.deltaTime);
                lurch = Mathf.Lerp(lurch, targetLurch, damp);

                var angles = playerCamera.transform.localEulerAngles;
                playerCamera.transform.localEulerAngles = new Vector3(angles.x, angles.y, -lurch);
            }
                ...
                if (isSprinting)
                {
                    ...
                    if (!unlimitedSprint)
                    {
                        ...
                        if (sprintRemaining <= 0)
                        {
                            ...
                            currentSprintCooldownDelay = sprintCooldownDelay;
                        }
                    }
                }
                else if (currentSprintCooldownDelay > 0)
                {
                    currentSprintCooldownDelay -= Time.deltaTime;
                    currentSprintCooldownDelay = Mathf.Max(0, currentSprintCooldownDelay);
                }
                else
                {
        ...
        void FixedUpdate()
        {
            ...
            if (playerCanMove)
            {
                ...
                Vector3 targetVelocity = new Vector3(_playerInput.Movement.Value.x, 0, _playerInput.Movement.Value.y);
                ...
                if (enableSprint && _playerInput.Sprinting.Value && sprintRemaining > 0f && !isSprintCooldown)
                {
        ...
        private void HeadBob()
        {
            if (isWalking)
            {
                ...
                float bobPosition = Mathf.Sin(timer);
                joint.localPosition = new Vector3(jointOriginalPos.x + bobPosition * bobAmount.x, jointOriginalPos.y + bobPosition * bobAmount.y, jointOriginalPos.z + bobPosition * bobAmount.z);
            }
            else
            {
    ...
    public class FirstPersonControllerEditor : Editor
    {
        ...
        public override void OnInspectorGUI()
        {
            ...
            fpc.canCameraLurch = EditorGUILayout.ToggleLeft(new GUIContent("Camera Lurch"), fpc.canCameraLurch);

            if (fpc.canCameraLurch)
            {
                EditorGUI.indentLevel++;
                fpc.cameraLurchStrength = EditorGUILayout.Slider(new GUIContent("Lurch Strength"), fpc.cameraLurchStrength, 0f, 0.2f);
                fpc.cameraLurchDamping = EditorGUILayout.Slider(new GUIContent("Lurch Damping"), fpc.cameraLurchDamping, .01f, 1f);
                EditorGUI.indentLevel--;
            }
            ...
            fpc.sprintCooldown = EditorGUILayout.Slider(new GUIContent("Sprint Cooldown", "Determines how long the recovery time is when the player runs out of sprint."), fpc.sprintCooldown, .1f, fpc.sprintDuration + fpc.sprintCooldownDelay);
            fpc.sprintCooldownDelay = EditorGUILayout.Slider(new GUIContent("Sprint Cooldown Delay"), fpc.sprintCooldownDelay, 0f, 2f);
            //GUI.enabled = true;
